...
addon = xbmcaddon.Addon('plugin.video.MundoDisney')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.MundoDisney-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(plugin.video.HINOJOSA,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(plugin.video.HINOJOSA,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
